...
if __name__=='__main__':
    ...
    im_src = cv2.imread(src).astype('float32')

    im_dst = cv2.imread(target).astype('float32')
    ...
    im_actual_L = cv2.cvtColor(im_src.astype('uint8'),cv2.BGR2Lab)[:,:,0]
    im_alpha_L = cv2.cvtColor(im_alpha.astype('uint8'),cv2.BGR2Lab)[:,:,0]
    im_poisson_L = cv2.cvtColor(im_res.astype('uint8'),cv2.BGR2Lab)[:,:,0]
